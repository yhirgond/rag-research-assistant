...
with tab1:
    ...
    query = st.text_input("Your Question:", placeholder="What methodology does this paper use?")
    ...
    if st.button("🔍 Get Answer", type="primary") and query:
        if store.is_empty():
            ...
            for i, chunk in enumerate(results, 1):
                with st.expander(f"[{i}] {chunk['source']} | Page {chunk['page']} | Score: {chunk['score']:.3f}"):
                    st.write(chunk["text"])
